...
if(life_inv_turtle == 0) exitWith {
	titleText["Du hast keine Schildkröten zum verkauf dabei.","PLAIN"];
};
...
if([false,"turtle",life_inv_turtle] call life_fnc_handleInv) then {
	titleText[format["Du hast %1 Schildkörten für $%2 verkauft.",_val,[_price] call life_fnc_numberText],"PLAIN"];
