...
_itemInfo = switch (_type) do
{
	...
	case "meth2": {["crystal_meth","crystal_meth_pure",3500,"Processing unpure Meth"]};
	default {[]};
};
...
if(_vendor in [mari_processor,mari_processor_2,coke_processor_1,coke_processor_2,coke_processor_3,hero_processor,hero_processor_2]) then {
	_hasLicense = true;
} else {
...
if(_hasLicense) then
{
	_zufall = ((round (random 5)) / 10) + 0.01;
	while{true} do
	{
		sleep _zufall;
		_cP = _cP + 0.01;
