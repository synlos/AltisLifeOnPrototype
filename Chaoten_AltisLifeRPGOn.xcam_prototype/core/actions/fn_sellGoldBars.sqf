...
if(life_inv_goldbar == 0) exitWith {
	titleText["Du hast keine Goldbaren zum Verkaufen dabei.","PLAIN"];
};
...
if([false,"goldbar",life_inv_goldbar] call life_fnc_handleInv) then {
	titleText[format["Du hast %1 Goldbaren für $%2 verkauft.",_val,[_price] call life_fnc_numberText],"PLAIN"];
