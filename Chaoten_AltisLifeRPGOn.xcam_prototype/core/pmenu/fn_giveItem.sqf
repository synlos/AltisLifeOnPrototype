...
_val = ctrlText 2010;
ctrlShow[2002,false];
if((lbCurSel 2023) == -1) exitWith {hint "Es war nichts ausgewählt!";ctrlShow[2002,true];};
_unit = lbData [2023,lbCurSel 2023];
_unit = call compile format["%1",_unit];
if((lbCurSel 2005) == -1) exitWith {hint "Du musst ein Item auswählen!";ctrlShow[2002,true];};
...
if(!([_val] call fnc_isnumber)) exitWith {hint "Du hast keine richtige Zahl eingegeben.";ctrlShow[2002,true];};
if(parseNumber(_val) <= 0) exitWith {hint "Du musst eine richtige Menge angeben!";ctrlShow[2002,true];};
if(isNil "_unit") exitWith {ctrlShow[2001,true]; hint "Der Spieler den du ausgewählt hast ist nicht in Reichweite";};
if(!([false,_item,(parseNumber _val)] call life_fnc_handleInv)) exitWith {hint "Hast du vieleicht zu wenig Items ?";ctrlShow[2002,true];};
