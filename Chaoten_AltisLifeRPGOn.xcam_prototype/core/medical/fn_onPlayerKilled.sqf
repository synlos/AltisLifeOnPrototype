...
diag_log format["DEBUG:: Server:: onPlayerKilled _unit spawn:36 called."];
_unit spawn
{
...
diag_log format["DEBUG:: Server:: onPlayerKilled deathScreen:52 called."];

//Create a thread to follow with some what precision view of the corpse.
diag_log format["DEBUG:: Server:: onPlayerKilled: Create a thread to follow with some what precision view of the corpse.:57"];
[_unit] spawn
{
	private["_unit"];
...
if(side _killer == west && playerSide != west) then {
	...
	if(!life_use_atm && {life_cash > 0}) then {
		[format["$%1 vom Raub der Staatsbank wurden zurückgebracht, da der Räuber umgekommen ist.",[life_cash] call life_fnc_numberText],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		life_cash = 0;
	};
};
...
diag_log format["DEBUG:: Client:: onPlayerKilled finished."];
