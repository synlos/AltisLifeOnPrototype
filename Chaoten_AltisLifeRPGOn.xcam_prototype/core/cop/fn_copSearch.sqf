...
if(count _invs > 0) then
{
	...
	[[0,format["%1 hat einen Wert von $%2 an Schmugelware dabei.",(_civ getVariable["realname",name _civ]),[_illegal] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
}
	else
{
...
if(_robber) then
{
	[[0,format["%1 wurde als Bankräuber entlarvt.",(_civ getVariable["realname",name _civ])]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};