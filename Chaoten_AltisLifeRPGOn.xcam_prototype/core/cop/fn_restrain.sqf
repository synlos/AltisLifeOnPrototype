...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			detach player;
			titleText["Du wurdest automatisch freigelassen, da du eine sehr lange Zeit gefesselt warst.","PLAIN"];
		};
	};
};

titleText[format["Du wurdest von %1 gefesselt!",_cop getVariable["realname",name _cop]],"PLAIN"];
