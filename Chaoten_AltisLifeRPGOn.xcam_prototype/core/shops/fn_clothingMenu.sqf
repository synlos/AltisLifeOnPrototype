...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart"] && playerSide != civilian) exitWith {hint "Nur ein Zivilist kann diesen Shop benutzen!"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Du besitzt kein Rebellen-Training"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint "Nur Polizisten können diesen Shop benutzen!"; closeDialog 0;};
if((_this select 3) in ["dive"] && !license_civ_dive) exitWith { hint "Du benötigst einen Führerschein, um auf diesen Shop zugreifen zu können"; closeDialog 0;};
...
{
	if(!(missionNamespace getVariable (_var select 0))) exitWith {hint format["Du benötigst %1 um hier kaufen zu können!",[_var select 0] call life_fnc_varToStr]; closeDialog 0;};
};
