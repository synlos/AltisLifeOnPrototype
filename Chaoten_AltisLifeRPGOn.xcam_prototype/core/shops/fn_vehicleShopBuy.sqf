...
if(life_cash < _basePrice) exitWith {hint format["Du hast nicht genug Geld um dieses Fahrzeug kaufen zu können.\n\nEs fehlt: $%1",[_basePrice - life_cash] call life_fnc_numberText];};
if(!([_className] call life_fnc_vehShopLicenses) && _className != "B_MRAP_01_hmg_F") exitWith {hint "Du besitzt nicht die benötigte Lizenz!"};
...
if(_spawnPoint == "") exitWith {hint "Der Spawnpunkt ist blockiert!";};
...
if((life_veh_shop select 0) == "med_air_hs") then {
	...
	_vehicle setPosATL (_hs modelToWorld [-0.4,-4,12.5]);
	_vehicle lock 2;
