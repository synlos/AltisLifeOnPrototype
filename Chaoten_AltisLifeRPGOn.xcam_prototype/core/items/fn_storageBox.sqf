...
if(!(_house in life_vehicles)) exitWith {hint "Um es zu platziern musst du in deinem Haus sein."};
_containers = _house getVariable["containers",[]];
_houseCfg = [(typeOf _house)] call life_fnc_houseConfig;
if(count _houseCfg == 0) exitWith {}; //What the fuck happened?
if(count _containers >= (_houseCfg select 1)) exitWith {hint "Du hast das limit an Storage Containers in deinem Haus erreicht"};
...
if(_pos isEqualTo [0,0,0]) exitWith {hint "Dein Haus ist voll!"};
