...
if(isNull _vehicle) exitWith {hint "Du musst auf das Fahrzeug schauen, welches du betanken willst."};
if(!(_vehicle isKindOF "LandVehicle") && !(_vehicle isKindOf "Air") && !(_vehicle isKindOf "Ship")) exitWith {};
if(player distance _vehicle > 7.5) exitWith {hint "Du musst näher ans Fahrzeug"};

if(!([false,"fuelF",1] call life_fnc_handleInv)) exitWith {};
life_action_inUse = true;
_displayName = getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName");

_upp = format["Tanke %1",_displayName];
...
titleText[format["Du hast erfolgreich getankt %1",_displayName],"PLAIN"];
