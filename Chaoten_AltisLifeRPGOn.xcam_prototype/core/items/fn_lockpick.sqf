...
if(_isVehicle && _curTarget in life_vehicles) exitWith {hint "Du besitzt schon einen Schlüssel für dieses Fahrzeug"};
...
if(!isNil "_badDistance") exitWith {titleText["Du bist zu weit vom Ziel weg!","PLAIN"]; life_action_inUse = false;};
...
[[_curTarget],"life_fnc_CarAlarmSound",nil,true] spawn life_fnc_MP;
	...
	if(_dice < 30) then {
		...
		[[0,format["%1 wurde dabei gesehen ein Fahrzeug zu knacken!",profileName]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
