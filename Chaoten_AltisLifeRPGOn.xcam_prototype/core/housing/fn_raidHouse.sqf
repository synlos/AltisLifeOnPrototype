...
if(!([_uid] call life_fnc_isUIDActive)) exitWith {hint "Da die Person nicht Online ist kannst du das Haus nicht Raiden!"};
_houseInv = _house getVariable ["Trunk",[[],0]];
if(_houseInv isEqualTo [[],0]) exitWith {hint "Das Haus ist leer."};
life_action_inUse = true;

//Setup the progress bar
disableSerialization;
_title = "Druchsuche das Haus...";
...
5 cutText ["","PLAIN"];
if(player distance _house > 13) exitWith {life_action_inUse = false; titleText["Du bist zu weit vom Haus weggegangen!","PLAIN"]};
...
if(_value > 0) then {
	[[0,format["In einem Haus wurden Dorgen/illegale Gegenstände in einem Wert von $%1 entdeckt.",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
